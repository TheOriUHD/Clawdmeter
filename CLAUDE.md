...
firmware/src/
  ...
  ui.{h,cpp}                — UI: splash, usage, settings (4 sliding pages: clock segmented picker + toggles / brightness & sleep sliders + status toggle + pairing button / Weekly card: default-face picker built from the live limits + flip-interval slider / About). Tap toggles splash ⇄ usage. Navigation is a finger-driven swipe engine hooked on the LVGL input device (lv_indev_add_event_cb: PRESSED/PRESSING/RELEASED, not LV_EVENT_GESTURE): vertical drags move Usage ⇄ Settings (Settings lives below; down from any settings page = home), horizontal drags page within Settings; surfaces follow the finger 1:1 and ONE lv_anim drives both to the snap (commit past span/4 or a flick). Presses starting on a slider are left alone. A drag arms the click guard because LVGL still sends CLICKED on release. Controls glide via animate_x() (ANIM_SEG_MS/ANIM_CTRL_MS); `transitioning` blocks presses mid-snap. Sim: SIM_DRAG="x0,y0,x1,y1,start_ms,dur_ms[,hold_ms]" scripts a finger for headless mid-drag captures. compute_layout() picks fonts/positions from board_caps() (breakpoints: H >= 460 large, H >= 300 compact, else small). The Weekly card has FACES: face 0 = all models, face i = scoped model i (Fable) rendered with the identical full-size card; auto-flip at the user's interval (settings face_flip; Off = tap picks and persists face_default), tap flips + holds while flipping — see render_weekly_face()/flip_weekly_face(). Accent = the one "active" colour for controls. Controls are ≥ 13 mm touch targets on the 39 mm 2.16" panel; never go back to 46 px rows.
  settings.{h,cpp}          — NVS-backed user prefs (clock mode, battery/mascot/status visibility, sleep timeout, Weekly-card flip interval + default face); pushes the sleep timeout into idle. brightness.cpp stores a continuous level ("brt_lvl", migrates the old preset index) in the same "clawdmeter" namespace; the slider previews live and persists on release, the PWR button steps presets.
