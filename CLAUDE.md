...
**Sound on the C6 2.16.** ES8311 at 0x18 on the shared I2C bus; I2S MCLK 19 / BCLK 20 / WS 22 / DIN 21 / DOUT 23; no GPIO amp enable (Waveshare's own 07_Audio_Test runs this board with `pa: -1`). `sound_hal_init()` reports `chime: ES8311 ready`. Alerts use codec volume 46 (the reset bell 65). Speaker output itself needs a human ear — confirm on the device.
