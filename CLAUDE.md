...
**Companion + Trend (macOS daemon, this fork):** `companion.py` (hook listener on 127.0.0.1:47393, session state machine, `"cc"` summary) and `trend.py` (poll history → `"tr"`) are imported by `claude_usage_daemon.py`; config keys `companion`, `companion_port`, `companion_bind`, `trend`. Tests: `daemon/tests/test_companion.py`, `test_trend.py`, `test_hooks_install.py` (run the suite with the scratch venv: `python -m pytest daemon/tests -q`, 169 pass). The Windows daemon has the companion too (same modules, `link` subcommand).
