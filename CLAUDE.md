...
firmware/src/
  ...
  ui.{h,cpp}                — UI: splash, usage, settings (3 sliding pages: clock segmented picker + toggles / brightness & sleep sliders + status toggle + pairing button / About). Tap toggles splash ⇄ usage; horizontal swipes slide usage → settings p1 → p2 → p3 (gesture handler on the LVGL screen object, click guard because LVGL sends CLICKED after a gesture; sliders clear GESTURE_BUBBLE so drags don't page). Motion: slide()/animate_x() with ease-out, ANIM_PAGE_MS/ANIM_CTRL_MS; `transitioning` blocks input mid-slide. compute_layout() picks fonts/positions from board_caps() (breakpoints: H >= 460 large, H >= 300 compact, else small). The Weekly card has FACES: face 0 = all models, face i = scoped model i (Fable) rendered with the identical full-size card; auto-flip every FACE_AUTO_MS, tap flips + holds — see render_weekly_face(). Accent = the one "active" colour for controls. Controls are ≥ 13 mm touch targets on the 39 mm 2.16" panel; never go back to 46 px rows.
